...
function uc_user_integral2($uid){
    $return = call_user_func(UC_API_FUNC,'user','get_integral2',array('uid'=>$uid));
    return uc_unserialize($return);
}
...
function uc_user_edit_integral2($uid,$type,$integral) {
	$return = call_user_func(UC_API_FUNC, 'user', 'integral_edit2', array('uid'=>$uid,'type' => $type,'integral' => $integral));
	return $return;
}
...
function uc_user_checknickname($nickname) {     //检查昵称是否已使用
	return call_user_func(UC_API_FUNC, 'user', 'check_nickname', array('nickname'=>$nickname));
}
function uc_user_nickname_edit($nickname,$uid) {    //修改用户中心昵称
	return call_user_func(UC_API_FUNC, 'user', 'edit_nickname', array('nickname'=>$nickname,'uid' => $uid));
}
...
function uc_user_behavior($uid){
    $return = call_user_func(UC_API_FUNC,'user','get_behavior',array('uid'=>$uid));
    return uc_unserialize($return);
}

function uc_user_get_search($where,$order,$limit) {
    $return = call_user_func(UC_API_FUNC, 'user', 'get_search', array('where'=>$where,'order' => $order,'limit' => $limit));
    return uc_unserialize($return);
}
