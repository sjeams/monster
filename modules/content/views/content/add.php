...
<div class="span10" id="datacontent">
    ...
    <form action="<?php echo baseUrl?>/content/content/add" method="post" class="form-horizontal" onsubmit="return checkContent();">
        <fieldset>
            <?php if(!isset($catId)) { ?>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <?php
                        ...
                        ?>
                        <input type="text"  class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="value[]" value="<?php echo empty($v['value'])&&isset($v['dataValue'])?$v['dataValue']:isset($v['value'])?$v['value']:date("Y-m-d H:i:s",time()); ?>" datatype=""
                               needle="needle" msg="">
                        <input type="hidden" name="key[]" value="<?php echo $v['id'] ?>">
                    <?php
                    }elseif($v['type'] == 0){
                    ...
                    <?php
                        ...
                        ?>
                        <textarea class="input-xxlarge" rows="7" 
                                    name="value[]"><?php echo empty($v['value'])&&isset($v['dataValue'])?$v['dataValue']:$v['value']?></textarea>
                        <?php if($v['name'] == '简介' || $v['name'] == '文章标签'){
                            echo '<input type="hidden" class="con" value="'.$v['name'].'" />';
                        }else{
                            echo '<input type="hidden"  value="'.$v['name'].'" />';
                        }?>
                        <input type="hidden" name="key[]" value="<?php echo $v['id'] ?>">
                    <?php
                    }
...
<script>
    ...
    function checkContent(){
        var res = $('.input-xxlarge').siblings('input').hasClass("con");
        if(res){
            var code = 1;
            $('input.con').each(function(e,row){
                var val = $(row).siblings('textarea').val();
                var name = $(row).val();
                val = val.trim();
                if(val == '' || !val){
                    alert('请填写'+name);
