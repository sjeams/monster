...
<div class="span10" id="datacontent">
    ...
    foreach($blocks as $val) {
        if ($val['value'] != 'add' && $val['value'] != 'sort' && $val['value'] != 'update' && $val['value'] != 'extend' && $val['value'] != 'delete') {
            ?>
            <a class="btn"
               href="<?php echo baseUrl ?>/content/content/<?php echo $val['value'] ?>?url=<?php echo Yii::$app->request->getUrl() ?>"><?php echo $val['name'] ?></a>
        <?php }
    }
    ?>

    <a class="btn" href="<?php echo baseUrl?>/content/content/index?hot=<?php echo $hot?>&pid=<?php echo $id?>">
        <?php if($hot==2){
            echo "取消热门";
        } else {
            echo "设置热门";
