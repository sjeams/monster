...
//                MockResult::updateAll(['sectionId'=>$seId,'hasTime'=>$time],"uid = $uid and mockExamId = $mockExamId and status =0");
    ...
    public function actionKnowCategory(){
        ...
        if(!$categoryId){
            ...
            $model = new Content();
            $contents = $model->getKnowList($categoryId);
            ...
            $data = ['code'=>1,'message'=>'success','data'=>['catName'=>$catName,'contents'=>$cons]];
        }
        die(json_encode($data));
    }
    /**
...
//        $day = date("Y-m");
    ...
    public function actionPreemptiveCourse(){
        ...
        $res['courses'] = $courseModel->getCourse();   //课程
        ...
        $res['auditionCourse'] = $courseModel->auditionCourse();
    ...
    public function actionTeacherList(){
        $teacherAll = Teacher::find()->asArray()->orderBy("sort desc")->all();
        die(json_encode(['code'=>1,'data'=>$teacherAll,'message'=>'请求成功']));
    }
    /**
     * 专家讲师详情页
     */
