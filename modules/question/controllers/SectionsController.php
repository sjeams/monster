...
class SectionsController extends App1Control {
    ...
    public function actionIndex()
    {
        ...
        $data = $model->getSections($page,$pageSize);
    ...
    public function actionAdd(){
        if($_POST){
            ...
            $re = Yii::$app->db->createCommand()->insert('{{%question_cat}}',$data)->execute();
            if($re){
                return $this->redirect('/question/sections/index');
            }else{
    ...
    public function actionUpdate(){
        if($_POST){
            ...
            QuestionCat::updateAll($data,"id=$id");
            $page = Yii::$app->session->get('page');
            $this->redirect("/question/sections/index?page=$page");
        }else{
            $id = Yii::$app->request->get('id');
            Yii::$app->session->set('page',Yii::$app->request->get('page'));
            $data = QuestionCat::find()->asArray()->where("id=$id")->one();
