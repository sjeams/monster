...
//        $order = isset($select['order'])?$select['order']:'c.sort ASC,c.id DESC';
        ...
        if(isset($select['pageStr'])){
            ...
            $content['data'] = $res;
        }else if(isset($select['all'])&&$select['all']==1 ){ //查询所有
            $content = \Yii::$app->db->createCommand("$sql ORDER BY $order")->queryAll(); 
        } else {
...
//        array_multisort($ages, SORT_DESC, $data);
    ...
    // 刷题活动查询
    // by sjeam
    public static function getTopicactionList($page,$pageSize,$all='',$like=''){
        if($like){ $where=" c.name like '%$like%' and " ;}else{$where='';}
        // $limit = " limit ".($page-1)*$pageSize.",$pageSize";
        // // 根据 属性 报名时间排序
        // $seField .= " SELECT ed.id From {{%extend}} as ce left JOIN {{%content_extend}} ed ON ed.extendid=ce.id where title like '%活动开始时间%' and ed.contentid=c.contentid ";
        // $where = " c.contentcatid =$catid and ed.id in ( $seField ) ";
        // $sql ="select c.*,ed.value as startTime from {{%content}} c left join {{%content_extend}} as ed on ed.contentid=c.contentid  where $where";
        // $count =count(Yii::$app->db->createCommand($sql)->queryAll());
        // $date ['count'] = $count;
        // $date ['total'] = ceil($count/$pageSize);
        // // 分页排序
        // $sql.=$order.$limit;
        // $res =  Yii::$app->db->createCommand($sql)->queryAll();
        $res = Content::getClass(['fields' => 'answer,cnName,duration,A,price,orderNum,article','where' =>"$where c.catid = 596",'order'=>'updateTime desc','page'=>$page,'pageSize'=>$pageSize,'all'=>$all]);   // 查找课程全部子内容
        // var_dump($res);die;
        // 获取课程属性
        $res =  Content::getBrushactiveExtend($res);

        // 订单数 ---是否报名和报名数
        $res = Content:: getOpenOrderNum($res);
        // $res[0]['teacher']='Bella';
        // 获取教师头像
        $date ['data'] = Teachers:: getOpenTeacherImage($res);
        // var_dump($date);die;
        return $date;
    ...
    * by sjeam
    */
    public static function getOpenOrderNum($res){
    ...
    // app 刷题活动属性查询
    // by sjeam
    public static function getBrushactiveExtend($res){
        foreach($res as $k=>$v){
            $res[$k]['price'] =  intval($v['price']);   // Content::getOpenContentName($v['contentid'],'价格')['value'];
            $res[$k]['teacher'] =   $v['A'];   unset($res[$k]['A']);  // Content::getOpenContentName($v['contentid'],'老师')['value'];
            $res[$k]['enorllStartTime'] =   $v['duration'];   unset($res[$k]['duration']);  // Content::getOpenContentName($v['contentid'],'活动开始时间')['value']; //活动时间
            $res[$k]['enorllEendTime'] =    $v['cnName'];   unset($res[$k]['cnName']);  // Content::getOpenContentName($v['contentid'],'活动结束时间')['value'];
            $res[$k]['activetype'] =    $v['answer'];   unset($res[$k]['answer']);  // Content::getOpenContentName($v['contentid'],'刷题类型')['value'];//1文章 2填空 3数学 4阅读 5单词 
            $res[$k]['orderNum'] =  intval($v['orderNum']);    // Content::getOpenContentName($v['contentid'],'订单数')['value']; //购买人数（订单数后台添加数量）
            $res[$k]['activetypeImg'] = Yii::$app->params['brushImg'][$res[$k]['activetype']];
            
            // 状态 3 未开始  2 正在  1结束
            strtotime($res[$k]['enorllStartTime']) < time() ? $res[$k]['status']= 3 : ( strtotime($res[$k]['enorllEendTime']) > time() ? $res[$k]['status']=2 : $res[$k]['status']=1 );
            // 改时间格式
            // $res[$k]['enorllStartTime'] =  date('Y-m-d', strtotime($res[$k]['enorllStartTime'] ));
            // $res[$k]['enorllEendTime']  =  date('Y-m-d', strtotime($res[$k]['enorllEendTime'] ));
            $res[$k]['timeExtend']  = date('Y-m-d', strtotime($res[$k]['enorllStartTime'] )). '至'.date('Y-m-d', strtotime($res[$k]['enorllEendTime'] ));
        }
        return $res;
    }


    // app 公开课（刷题活动） 公用name 查询方法
    // by sjeam
    // public static function getOpenContentName($contentid,$name){
    //     $sql=" select c.value from {{%extend}} as e left join {{%content_extend}} as c on e.id=extendid where title like '%$name%' and contentid = $contentid ";
     ...
     * by  sjeam
     */
     public static function teacherList($where){
        $data = Yii::$app->db->createCommand("select * from x2_teachers $where order by sort desc")->queryAll();
  
        foreach($data as $k => $v){
            $id = $v['id'];
            $time = time();
            if(!$v['havenumber'] || $v['havenumber'] < 60){
                $havenumber = rand(60,100);
            }else{
                $gettime = $time - strtotime( $v['createTime']);
                $days = floor($gettime/86400);
                $adds = rand(3,6)*$days;
                $havenumber = $v['havenumber'] + $adds;
            }
            Yii::$app->db->createCommand("update x2_teachers set havenumber = $havenumber where id = $id")->execute();
            $data[$k]['havenumber'] = (1000+$havenumber);
            $data[$k]['gettime'] = date('Y-m-d',$time);
        }
        return $data;
    }
