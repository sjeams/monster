...
class Course extends ActiveRecord {
    ...
    //获提分课程所有课  by sjeam 
    public static function getCourse($page,$pageSize,$all='',$like=''){  //$all==1  查询所有
        if($like){ $where=" c.name like '%$like%' and " ;}else{$where='';}
        $deputyCourse = Course::find()->asArray()->where("type!=2")->orderBy("sort desc")->all();
        // 新版查询所有课
        $contentid=  array_column($deputyCourse,'contentid');
        $contentid= implode($contentid,',');
        $courseContent = Content::getClass(['fields' => 'alternatives,commencement,performance,duration,price','where' =>"  $where c.id in($contentid)",'order'=>'createTime desc','page'=>$page,'pageSize'=>$pageSize,'all'=>$all]);   // 查找课程全部子内容
        foreach($courseContent as $k=>$v){
            $courseContent[$k]['place'] = \Yii::$app->params['coursePlace'][$v['id']%3]; // 随机教室
        }
    ...
    public static function auditionCourse(){
        $data['blank'] = [
            'name'=>'填空试听',
            'url'=>'m49GmiJg7u',
            'time'=>'半个小时',
